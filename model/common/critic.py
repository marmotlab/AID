...
class GraphCritic(CriticObs):
    """GraphAttentionEncoder + MLP, state only"""
    ...
    def forward(self, cond: dict):
        ...
        TODO long term: more flexible handling of cond
        """
        first_key = list(cond.keys())[0]
        sample_data = cond[first_key] # doesnt matter what key is used, batch size is same
        B = len(sample_data) # any key will do since all same batch size

        node_inputs = einops.rearrange(cond["node_inputs"], "b h n d -> (b h) n d")
        pos_embedding = einops.rearrange(cond["pos_embedding"], "b h n d -> (b h) n d")
        agent_inputs = einops.rearrange(cond["agent_inputs"], "b h n d -> (b h) n d")
        output_features = self.backbone(node_inputs, pos_embedding, agent_inputs)

        # augmented_graph = einops.rearrange(cond["augmented_graph"], "b h n d -> (b h) n d")
        # pos_embedding = einops.rearrange(cond["pos_embedding"], "b h n d -> (b h) n d")
        # current_position = einops.rearrange(cond["current_position"], "b h d -> (b h) d")
        # budget = einops.rearrange(cond["budget"], "b h d -> (b h) d")
        # output_features = self.backbone(augmented_graph, pos_embedding, current_position, budget) # (B * To, D)
        feat = output_features.view(B, -1)
        return super().forward(feat)