...
class PRMController:
    ...
    def runPRM(self, coordinates, seed=None, start_pos=None, saveImage=False):
        if saveImage:
            utils = Utils()
            utils.drawMap(self.allObs, self.start, self.destination)
        if seed is not None:
            print(f"PRM seed: {seed}")
            np.random.seed(seed)

        # Generate n random samples called milestones
        self.genCoords(start_pos=start_pos, coordinates=coordinates)
    ...
    def genCoords(self, start_pos, coordinates):
