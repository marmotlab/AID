...
class EvalDiffusionWorker:
    def __init__(self,
                 meta_agent_id, agent_id, episode_number, env, shared_memory, lock, num_agent,
                 diffusion_model, cond_steps, num_paths, action_steps, budget,
                 steps, data_type, normalization_path=None, step_size=0.1, device='cpu', save_image=False, gifs_path='gifs'):
    ...
    def run_episode(self):
        ...
        for step in range(self.steps):
            ...
            # Get planned path
            with torch.no_grad():
                planned_path = self.diffusion_model(cond)[0].cpu().detach().numpy() # (num_paths, horizon, 2)
            ...
            best_planned_path = planned_path[best_path_id] # (action_steps, 2)
    ...
    def construct_cond(self):
        ...
        agent_inputs_cond = torch.tensor(np.array(self.agent_inputs_queue), dtype=torch.float32).unsqueeze(0).to(self.device) # (1, cond_steps, 4)
        ...
        agent_inputs_cond = agent_inputs_cond.repeat(num_paths, 1, 1, 1) # (num_paths, cond_steps, 4)
    ...
    def plot(self, obs, gaussian_mean, gaussian_cov, planned_trajectory=[], all_planned_trajectory=[]):
        ...
        M = 1000  # sample numbers in gaussian distribution
        gaussian_value = np.zeros((M, M))
        mean = gaussian_mean[f"{self.agent_id}"]
        cov = gaussian_cov[f"{self.agent_id}"]
        if len(mean) != 0:
            Gaussian = multivariate_normal(mean=mean, cov=cov)
            X, Y = np.meshgrid(np.linspace(0, 1, M), np.linspace(0, 1, M))
            d = np.dstack([X, Y])
